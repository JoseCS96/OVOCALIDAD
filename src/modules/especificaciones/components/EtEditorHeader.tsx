...
import EtWorkflowActions from "./EtWorkflowActions";

type Props={info:InformacionGeneralEt;guardando:boolean;ultimoGuardado:Date|null;confirmando:boolean;workflowBusy:boolean;onVolver:()=>void;onGuardarBorrador:()=>void;onWorkflow:(accion:AccionWorkflowEt,comentario:string|null)=>void};
export default function EtEditorHeader({info,guardando,ultimoGuardado,confirmando,workflowBusy,onVolver,onGuardarBorrador,onWorkflow}:Props){
 return <div className="flex items-start justify-between gap-4">
  ...
  <div className="flex items-center gap-3"><div className="text-right text-xs text-[var(--text-secondary)]">{guardando?"Guardando cambios...":ultimoGuardado?`Cambios guardados · ${ultimoGuardado.toLocaleTimeString("es-PE",{hour:"2-digit",minute:"2-digit"})}`:"Autoguardado activo"}</div>
   <EtWorkflowActions info={info} busy={workflowBusy} onAccion={onWorkflow}/><Button className="h-11 bg-slate-900 px-5 text-base font-semibold text-white shadow-md hover:bg-slate-800 hover:shadow-lg" disabled={!info.permiteEditar||confirmando||guardando} onClick={onGuardarBorrador}><Save className="size-5"/>{confirmando?"Confirmando...":"Guardar borrador"}</Button>

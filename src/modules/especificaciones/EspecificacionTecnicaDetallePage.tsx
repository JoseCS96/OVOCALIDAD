...
export default function EspecificacionTecnicaDetallePage(){
 ...
 const secs=useMemo(()=>[...(q.data?.secciones??[])].sort((a,b)=>(a.orden??999)-(b.orden??999)),[q.data]),act=secs.find(x=>x.seccionId===(sel??secs[0]?.seccionId));
...
function Cars({a}:{a:CaracteristicaEt[]}){if(!a.length)return <Vacio/>;return <div className="space-y-6">{Array.from(new Set(a.map(x=>x.tipoCaracteristica))).map(g=><div key={g}><h3 className="mb-2 font-semibold">{cap(g)}</h3><T h={["Característica","Obligatoria","Criterio","Especificación","Unidad","Método"]} r={a.filter(x=>x.tipoCaracteristica===g).sort(ord).map(x=>[x.caracteristica,x.esObligatorio?"Sí":"No",x.tipoCriterio??"—",spec(x),x.unidad??"N.A.",x.metodoEnsayo??"N.A."])}/></div>)}</div>}
